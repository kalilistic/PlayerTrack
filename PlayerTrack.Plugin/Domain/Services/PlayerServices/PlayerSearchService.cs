...
public static class PlayerSearchService
{
    ...
    public static Func<Player, bool> GetSearchFilter(string searchString, SearchType searchType)
    {
        ...
        var tokens = searchString.Split([' '], StringSplitOptions.RemoveEmptyEntries);
    ...
    private static Func<Player, bool> CreateSingleFilter(string token, SearchType searchType)
    {
        ...
        if (Handlers.TryGetValue(key, out var h))
        {
            return player =>
            {
                var result = value == "!" ? !h.hasValue(player) : h.match(player, value);
    ...
    private static (bool isNegated, string key, string value) ParseSearchString(string searchString)
    {
        ...
        if (parts.Length != 2)
            return (isGloballyNegated, "default", normalized);
        ...
        return (isGloballyNegated || isValueNegated, key, value);
    }
    ...
    private static bool MatchPrefix(string field, string pattern)
    {
        ...
        {
            "*" => true,
            not null when pattern.StartsWith('*') && pattern.EndsWith('*') => field.Contains(pattern.Trim('*'), StringComparison.OrdinalIgnoreCase),
            not null when pattern.StartsWith('*') => field.EndsWith(pattern.TrimStart('*'), StringComparison.OrdinalIgnoreCase),
            not null when pattern.EndsWith('*') => field.StartsWith(pattern.TrimEnd('*'), StringComparison.OrdinalIgnoreCase),
