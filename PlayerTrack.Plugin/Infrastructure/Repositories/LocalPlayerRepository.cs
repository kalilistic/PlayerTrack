...
{
    public LocalPlayerRepository(IDbConnection connection, IMapper mapper) : base(connection, mapper) { }

    public List<LocalPlayer> GetAllLocalPlayers()
    {
        const string sql = "SELECT * FROM local_players";
        var localPlayerDTOs = Connection.Query<LocalPlayerDTO>(sql).ToList();
        return localPlayerDTOs.Select(dto => Mapper.Map<LocalPlayer>(dto)).ToList();
    }

    public LocalPlayer? GetLocalPlayer(ulong contentId)
    {
        const string sql = "SELECT * FROM local_players WHERE content_id = @contentId";
        var localPlayerDTO = Connection.QuerySingleOrDefault<LocalPlayerDTO>(sql, new { contentId });
        return localPlayerDTO == null ? null : Mapper.Map<LocalPlayer>(localPlayerDTO);
    }

    public int CreateLocalPlayer(LocalPlayer localPlayer)
    {
        var localPlayerDTO = Mapper.Map<LocalPlayerDTO>(localPlayer);
        ...
        const string sql = @"
            ...
            RETURNING id;";
        return Connection.ExecuteScalar<int>(sql, localPlayerDTO);
    }
    ...
    public void UpdateLocalPlayer(LocalPlayer localPlayer)
    {
        ...
        const string sql = @"
            ...
            SET name = @name,
                ...
                updated = @updated
            WHERE id = @id;";
        Connection.Execute(sql, localPlayerDTO);
    }
