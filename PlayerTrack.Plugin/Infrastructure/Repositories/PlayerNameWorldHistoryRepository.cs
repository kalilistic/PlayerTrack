...
{
    public PlayerNameWorldHistoryRepository(IDbConnection connection, IMapper mapper) : base(connection, mapper) { }
    ...
    public int CreatePlayerNameWorldHistory(PlayerNameWorldHistory playerNameWorldHistory)
    {
        ...
        var historyDto = Mapper.Map<PlayerNameWorldHistoryDTO>(playerNameWorldHistory);
        SetCreateTimestamp(historyDto);

        var newId = Connection.ExecuteScalar<int>(sql, historyDto);
        return newId;
    }

    public int UpdatePlayerId(int originalPlayerId, int newPlayerId)
    {
        Plugin.PluginLog.Verbose("Entering PlayerNameWorldHistoryRepository.UpdatePlayerId()");
        try
        {
    ...
    {
        Plugin.PluginLog.Verbose($"Entering PlayerNameWorldHistoryRepository.GetHistoricalNames(): {playerId}");
        try
        {
            const string sql = "SELECT player_name FROM player_name_world_histories WHERE player_id = @player_id ORDER BY updated DESC";
            return Connection.Query<string>(sql, new { player_id = playerId }).ToArray();
        }
        catch (Exception ex)
        {
            Plugin.PluginLog.Error(ex, $"Failed to get historical names for PlayerID {playerId}.");
            return null;
        }
    }
    ...
    {
        Plugin.PluginLog.Verbose($"Entering PlayerNameWorldHistoryRepository.GetHistoricalWorlds(): {playerId}");
        try
        {
            const string sql = "SELECT world_id FROM player_name_world_histories WHERE player_id = @player_id ORDER BY updated DESC";
            return Connection.Query<uint>(sql, new { player_id = playerId }).ToArray();
        }
        catch (Exception ex)
        {
            Plugin.PluginLog.Error(ex, $"Failed to get historical worlds for PlayerID {playerId}.");
            return null;
        }
    }
    ...
    {
        Plugin.PluginLog.Verbose($"Entering PlayerNameWorldHistoryRepository.GetPlayerNameWorldHistories(): {playerId}");
        try
        {
            const string sql = "SELECT * FROM player_name_world_histories WHERE player_id = @player_id ORDER BY updated DESC";
            return Connection.Query<PlayerNameWorldHistoryDTO>(sql, new { player_id = playerId }).Select(x => Mapper.Map<PlayerNameWorldHistory>(x));
        }
        catch (Exception ex)
        {
            Plugin.PluginLog.Error(ex, $"Failed to get player name world history for PlayerID {playerId}");
    ...
    public IEnumerable<PlayerNameWorldHistory>? GetPlayerNameWorldHistories(int[] playerIds)
    {
        ...
        Plugin.PluginLog.Verbose($"Entering PlayerNameWorldHistoryRepository.GetPlayerNameWorldHistories()");
        try
        {
            ...
            for (int pageStart = 0; pageStart < playerIds.Length; pageStart += maxPerQuery)
            {
                ...
                var pageResults = Connection.Query<PlayerNameWorldHistoryDTO>(sql).Select(x => Mapper.Map<PlayerNameWorldHistory>(x));
        ...
        {
            Plugin.PluginLog.Error(ex, $"Failed to get bulk player name world history");
            return null;
        }
    }
    ...
    public IEnumerable<PlayerNameWorldHistory>? GetAllPlayerNameWorldHistories() {
        ...
        try {
            ...
            return Connection.Query<PlayerNameWorldHistoryDTO>(sql).Select(x => Mapper.Map<PlayerNameWorldHistory>(x));
        }
        catch (Exception ex) {
            Plugin.PluginLog.Error(ex, $"Failed to get all player name world histories");
            return null;
        }
    }
    ...
    {
        Plugin.PluginLog.Verbose($"Entering PlayerNameWorldHistoryRepository.DeleteNameWorldHistory(): {playerId}");
        try
        {
        ...
        {
            Plugin.PluginLog.Error(ex, $"Failed to delete NameWorldHistory for PlayerID {playerId}");
            return false;
        }
    }
    ...
    {
        Plugin.PluginLog.Verbose("Entering PlayerNameWorldHistoryRepository.CreatePlayerNameWorldHistories()");
        ...
        VALUES (@created, @updated, @is_migrated, @player_name, @world_id, @player_id)";
            ...
            Connection.Execute(sql, historyDTOs, transaction);
        ...
        {
            Plugin.PluginLog.Error(ex, "Failed to create PlayerNameWorldHistories.");
