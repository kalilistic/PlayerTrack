...
{
    public SocialListRepository(IDbConnection connection, IMapper mapper) : base(connection, mapper) { }

    public SocialList? GetSocialList(int id)
    {
        const string sql = "SELECT * FROM social_lists WHERE id = @id";
        var socialListDTO = Connection.QuerySingleOrDefault<SocialListDTO>(sql, new { id });
        return socialListDTO == null ? null : Mapper.Map<SocialList>(socialListDTO);
    }
    ...
    public SocialList? GetSocialList(ulong contentId, SocialListType socialListType)
    {
        ...
        var socialListDTO = Connection.QuerySingleOrDefault<SocialListDTO>(sql, new { content_id = contentId, list_type = socialListType });
        return socialListDTO == null ? null : Mapper.Map<SocialList>(socialListDTO);
    }
    ...
    public List<SocialList> GetSocialLists(ulong contentId)
    {
        ...
        var socialListDTOs = Connection.Query<SocialListDTO>(sql, new { content_id = contentId }).ToList();
        return socialListDTOs.Select(dto => Mapper.Map<SocialList>(dto)).ToList();
    }
    ...
    public List<SocialList> GetSocialListsWithDefaultCategory(int categoryId)
    {
        ...
        var socialListDTOs = Connection.Query<SocialListDTO>(sql, new { default_category_id = categoryId }).ToList();
        return socialListDTOs.Select(dto => Mapper.Map<SocialList>(dto)).ToList();
    }
    ...
    public SocialList? GetSocialList(ulong contentId, SocialListType socialListType, ushort listNumber, uint dataCenterId)
    {
        ...
        var socialListDTO = Connection.QuerySingleOrDefault<SocialListDTO>(sql, new { content_id = contentId, list_type = socialListType, data_center_id = dataCenterId, list_number = listNumber });
        return socialListDTO == null ? null : Mapper.Map<SocialList>(socialListDTO);
    }
    ...
    public SocialList? GetSocialList(ulong contentId, SocialListType socialListType, ushort listNumber)
    {
        ...
        var socialListDTO = Connection.QuerySingleOrDefault<SocialListDTO>(sql, new { content_id = contentId, list_type = socialListType, list_number = listNumber });
        return socialListDTO == null ? null : Mapper.Map<SocialList>(socialListDTO);
    }

    public int CreateSocialList(SocialList socialList)
    {
        var socialListDTO = Mapper.Map<SocialListDTO>(socialList);
        ...
        return Connection.ExecuteScalar<int>(sql, socialListDTO);
    }
    ...
    public void UpdateSocialList(SocialList socialList)
    {
        ...
        const string sql = @"
            ...
            SET list_type = @list_type,
                ...
                updated = @updated
            WHERE id = @id;";
        Connection.Execute(sql, socialListDTO);
    }
