...
{
    public SocialListMemberRepository(IDbConnection connection, IMapper mapper)
    ...
    public List<SocialListMember> GetSocialListMembers(int socialListId)
    {
        ...
        var socialListMemberDTOs = this.Connection.Query<SocialListMemberDTO>(sql, new { social_list_id = socialListId }).ToList();
        return socialListMemberDTOs.Select(dto => this.Mapper.Map<SocialListMember>(dto)).ToList();
    }
    ...
    public List<SocialListMember> GetSocialListMembers(int socialListId, int pageNumber)
    {
        ...
        var socialListMemberDTOs = this.Connection.Query<SocialListMemberDTO>(sql, new { social_list_id = socialListId, page_number = pageNumber }).ToList();
        return socialListMemberDTOs.Select(dto => this.Mapper.Map<SocialListMember>(dto)).ToList();
    }
    
    public int CreateSocialListMember(SocialListMember member)
    {
        var socialListMemberDTO = this.Mapper.Map<SocialListMemberDTO>(member);
        ...
        const string sql = @"
            ...
            RETURNING id;";
        return this.Connection.ExecuteScalar<int>(sql, socialListMemberDTO);
    }
    ...
    public void DeleteSocialListMembers(int socialListId)
    {
        ...
        this.Connection.Execute(sql, new { id = socialListId });
    }
}